...
### Step 4: Merge & Push ไป Main (Git Sync + Auto Backup)

**⚠️ สำคัญ:** เมื่อ Merge staging → main จะมีการ **Backup Version Tag อัตโนมัติทุกครั้ง**
...
### Step 5: สร้าง Version Tag (Auto Backup)

**🤖 ระบบจะทำอัตโนมัติ:**
1. อ่าน commit messages ตั้งแต่ tag ล่าสุด
2. วิเคราะห์ประเภทการเปลี่ยนแปลง (ตาม Conventional Commits)
3. คำนวณ version number ใหม่ตาม Semantic Versioning
4. สร้าง tag และ push ขึ้น GitHub

**การวิเคราะห์ Version Type:**
```
Commit Message          → Version Type → ตัวอย่าง
─────────────────────────────────────────────────
BREAKING CHANGE: ...    → MAJOR        → v2.0.0
feat!: ...              → MAJOR        → v2.0.0
feat: เพิ่มฟีเจอร์       → MINOR        → v1.1.0
fix: แก้ bug            → PATCH        → v1.0.1
docs: อัปเดตเอกสาร      → PATCH        → v1.0.1
chore: ...              → PATCH        → v1.0.1
```
...
- **PATCH** (v1.0.1): Bug fixes หรือการเปลี่ยนแปลงเล็กน้อย

**ผลลัพธ์:** Version Tag สร้างและ backup บน GitHub อัตโนมัติ ✅
...
2. **Merge = Auto Backup**
   - ทุกครั้งที่ Merge staging → main จะมีการสร้าง Version Tag อัตโนมัติ
   - ระบบจะวิเคราะห์ commit messages และกำหนด version เอง
   - ใช้ Conventional Commits (feat:, fix:, BREAKING CHANGE:) เพื่อความถูกต้อง
...
4. **ใช้ Conventional Commits**
   ...
   - `fix:` สำหรับแก้ bug
   - `docs:` สำหรับเอกสาร
   - `chore:` สำหรับงานทั่วไป
   - `BREAKING CHANGE:` สำหรับการเปลี่ยนแปลงใหญ่
