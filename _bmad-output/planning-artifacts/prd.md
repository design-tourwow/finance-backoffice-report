...
- ✅ **Phase 6 follow-up — /canceled-orders RBAC expansion + cleanup** (2026-05-05 — same day) — งานต่อเนื่องจาก commit แรกของ Phase 6 ครอบคลุม access expansion, view-as fidelity bugs ที่ตรวจเจอตอนผู้ใช้เทส, และ feature เล็กๆ บน /sales-report-by-seller. รายละเอียด:
  - **/sales-report-by-seller — traveler-count toggle** — checkbox "นับ Order ที่มีผู้เดินทางเท่านั้น" บน toolbar เหนือตาราง (ใช้คลาส `banner1-filter-checkbox` ที่ tour-image-manager.css โหลดอยู่แล้ว). Checked (default) = `room_quantity > 0` เหมือนพฤติกรรม Phase 6 commit แรก; unchecked = `room_quantity = 0` เพื่อให้ finance audit Order ที่ผู้เดินทางเป็น 0 แยก. Filter ที่ entry เดียว (`renderResults`) — KPI / ranking / table / export ดูชุดเดียวกัน. Empty-page early-return ตอนนี้เช็ค `rawOrders` (API result) แทน post-filter เพื่อให้ toolbar ยังอยู่ตอนใช้ filter จนผลเหลือ 0
  - **/canceled-orders — เปิดสิทธิ์ ts/crm** — flip `ROLE_ACCESS['/canceled-orders']` จาก `false` → `true` สำหรับทั้ง `ts` และ `crm` ใน `menu-component.js`. Backend `commission-plus` รองรับ ['admin','ts','crm'] อยู่แล้ว. Page-level `isAdmin()` guards เดิมจัดการ disabled state ของ ตำแหน่ง / เซลล์ผู้จอง dropdown ให้อัตโนมัติ
  - **/canceled-orders — "ทั้งหมด" option บน วันที่สร้าง Order dropdown** — เพิ่มเป็นตัวเลือกแรก (default) ไม่ apply created_at filter — degrade เป็น "ทุก Order ที่ยกเลิกในช่วงนี้" ทำให้ ทั้งหมด = ก่อนช่วงที่ยกเลิก ∪ ตรงกับช่วงที่ยกเลิก (3 ตัวเลือกเป็น exhaustive partition)
  - **/canceled-orders — KPI rename** — "ยอดจองรวม" → "ยอดยกเลิกรวม" บน KPI card, print card label, และ PDF footer total (3 จุด). Per-row column header "ยอดจอง" ในตารางคงเดิม — ตัวเลขในแต่ละแถวยังคงสื่อถึง booking value ของ order นั้นๆ ก่อนถูกยกเลิก
  - **/canceled-orders — ลบ seller summary section** — page นี้ใช้สำหรับ audit/follow-up Order ที่ยกเลิก ไม่ใช่ ranking analytics; section เป็น admin-only อยู่แล้วและไม่มี business case ที่ใช้จริง. ลบ `renderSellerSummary()`, `buildSellerAggregate()`, `sortSellerAggregate()`, `sellerSummarySort` state, และ `.crp-summary-table` sortable binding (-107 lines)
  - **/canceled-orders — view-as awareness fix** — `isAdmin()` เดิมอ่าน `currentUser.job_position` ตรงๆ ไม่รู้จัก view-as state. Port helpers `getEffectiveRole()` / `getEffectiveUserId()` / `getEffectiveNickName()` จาก /sales-report-by-seller (single source of truth ที่ SOP กำหนด) — admin id=555 view-as ts/crm ตอนนี้เห็นค่าที่ locked ตรงกับผู้ที่ถูก impersonate
  - **/canceled-orders — own-seller scoping (parity bug fix)** — backend คืน role-wide rows มาให้ (per design — Section 7 ของ architecture-rbac-view-as.md) แต่ /canceled-orders เรนเดอร์ตรงๆ ไม่กรอง ทำให้ ts/crm เห็น Order ของคนอื่นในตาราง. เพิ่ม `ownOrders = orders.filter(seller_agency_member_id === effectiveUserId)` สำหรับ non-admin + computeSummary client-side; KPI / ตาราง / CSV / PDF ใช้ ownOrders+ownSummary ตรงกัน. มี module state ใหม่ `currentOwnOrders` / `currentOwnSummary` เพื่อให้ print code อ่านค่าหลัง filter แทนที่จะอ่านจาก backend summary
...
- **FR60:** /sales-report-by-seller toolbar (เหนือตารางหลัก) ต้องมี checkbox "นับ Order ที่มีผู้เดินทางเท่านั้น" — checked (default) = filter `room_quantity > 0`, unchecked = filter `room_quantity = 0`. Toggle apply ที่ entry เดียวของ `renderResults` ทุก consumer (KPI / ranking / ตาราง / export) sync ทันที ไม่ต้องยิง API ใหม่
- **FR61:** /canceled-orders ต้องเปิดให้ ts และ crm role เข้าถึงได้. ทำตาม pattern เดียวกับ /sales-report-by-seller: ตำแหน่ง + เซลล์ผู้จอง dropdown disabled และ locked ที่ผู้ใช้ตัวเอง; KPI / ตาราง / export กรองให้เหลือเฉพาะ Order ของ effective user (`seller_agency_member_id === getEffectiveUserId()`); page-level `isAdmin()` ต้อง view-as aware (ผ่าน `getEffectiveRole()`)
- **FR62:** /canceled-orders dropdown "วันที่สร้าง Order" ต้องมี "ทั้งหมด" เป็นตัวเลือกแรกและเป็น default — เลือกแล้วไม่ apply created_at filter เลย (degrade เป็นทุก Order ในช่วงที่ยกเลิก). อีก 2 ตัวเลือก ("ก่อนช่วงที่ยกเลิก" / "ตรงกับช่วงที่ยกเลิก") ยังคงพฤติกรรมเดิม
- **FR63:** /canceled-orders KPI / print / PDF total ต้องใช้คำว่า "ยอดยกเลิกรวม" (ไม่ใช่ "ยอดจองรวม") เพราะค่ารวมในหน้านี้คือ value ของ Order ที่ถูกยกเลิก ไม่ใช่ value ของ booking. Per-row column header "ยอดจอง" ในตารางคงเดิม
- **FR64:** /canceled-orders ไม่แสดง seller summary section (Telesales + CRM ranking groups) อีกต่อไป — page นี้ใช้สำหรับ audit/follow-up canceled orders ไม่ใช่ ranking analytics
