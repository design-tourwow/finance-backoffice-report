...
(function () {
  ...
  function computeSummary(orders) {
    return (orders || []).reduce(function (acc, o) {
      ...
      if (String(o.order_status || '').toLowerCase() === 'canceled') {
        acc.total_canceled_amount += parseFloat(o.net_amount || 0);
        acc.total_canceled_orders += 1;
      } else {
        acc.total_active_amount += parseFloat(o.net_amount || 0);
        acc.total_active_orders += 1;
      }
      return acc;
    }, { total_orders: 0, total_net_amount: 0, total_commission: 0, total_discount: 0,
         total_active_amount: 0, total_active_orders: 0,
         total_canceled_amount: 0, total_canceled_orders: 0 });
  }
  ...
  function buildCanceledReferenceNavUrl() {
  ...
  async function fetchCanceledReferenceData() {
  ...
  function renderSummary(summary) {
    ...
    const hasCanceledAmount = canceledReferenceTotal > 0;
    const canceledNote = canceledReferenceNavUrl
      ? `<div class="kpi-note-row">
           <span class="kpi-note">${hasCanceledAmount
             ? `มียอด Order ที่ยกเลิก ${formatNumber(canceledReferenceTotal, 0)} บาท`
             : 'ไม่มียอด Order ที่ยกเลิก'}</span>
           ${hasCanceledAmount ? `<a class="kpi-note-link" href="${escHtml(canceledReferenceNavUrl)}" title="ดูใน Canceled Orders" aria-label="ดูใน Canceled Orders">
    ...
    return `
      <div class="dashboard-kpi-cards">
        ...
        <div class="dashboard-kpi-card kpi-completed">
          ...
          <div class="kpi-content">
            <div class="kpi-label">ยอดสุทธิ (หักยกเลิก)</div>
...
<html lang="th">
  <head>
    <meta charset="UTF-8" />
    <title>Sales Report by Ching</title>
  ...
  <body>
    <div class="crp-print-shell">
      <div class="crp-print-header">
        <div>
          <h1 class="crp-print-title">Sales Report by Ching</h1>
  ...
  function createPdfSourceNode(countText, summary) {
    ...
    const totalDiscountPercent = getDiscountPercentValue(reportSummary.total_discount, reportSummary.total_net_amount);
    ...
    title.innerHTML = `
      <div>
        <div style="font-size:22px;font-weight:700;color:#0f172a;line-height:1.2;">Sales Report by Ching</div>
      </div>
    `;
