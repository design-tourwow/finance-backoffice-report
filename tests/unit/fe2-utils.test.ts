...
/**
 * Unit test sample (U1, U2). Loads fe2-utils.js into a sandboxed scope and
 * exercises pure helpers.
 *
 * Pattern: each unit test evaluates fe2-utils.js in a fake-window shim;
 * assertions run in Node. No browser required.
 */

function loadFE2Utils(): Record<string, any> {
  const src = readFileSync(path.join(__dirname, '..', '..', 'fe2-utils.js'), 'utf-8');
  const win: any = {};
